...
def mcom_dash(dp_object, dash_player, throughput, current_bitrate, adaptationSetIdx, segment_number):
    ...
    if dash_player.playback_state == "BUFFERING":
        config_dash.LOG.info("Player in buffering status -> Switching to low quality")
        bitrates = get_bitrates_for_mcom(dp_object, list(dp_object.adaptationSets[adaptationSetIdx].video.keys())[0], adaptationSetIdx)
        vmafs = get_vmafs_for_mcom(dp_object, list(dp_object.adaptationSets[adaptationSetIdx].video.keys())[0], adaptationSetIdx, segment_number)
        segment_sizes = get_segment_sizes_for_mcom(dp_object, list(dp_object.adaptationSets[adaptationSetIdx].video.keys())[0], adaptationSetIdx, segment_number)
    ...
    if alpha >= 1:
        alpha = 0.9  # Consider the size, even if minimally
    elif alpha <= 0.1:
        alpha = 0.1
    ...
    if bitrates:
        ...
        sel_objFunc = alpha * selVMAF / maxVMAF - (1 - alpha) * selSS / maxSS
                
        for adaptation in dp_object.adaptationSets:
        
            temp_objFunc = alpha * vmafs[int(adaptation.id)] / maxVMAF - (1 - alpha) * segment_sizes[int(adaptation.id)] / maxSS
            ...
            config_dash.LOG.info("Bitrate '{}' from adaptation with id '{}': segment size -> {} Bytes, VMAF = {}, Estimated Download time: {}s, Objective Function: {}".format(
                bitrates[int(adaptation.id)], adaptation.id, float(segment_sizes[int(adaptation.id)]) / 8, vmafs[int(adaptation.id)], float(segment_sizes[int(adaptation.id)]) / 1000 / throughput, temp_objFunc))
        
            if int(adaptation.id) == fixCodecAdaptationId:
                continue           
            
            if float(vmafs[fixCodecAdaptationId]) - float(vmafs[int(adaptation.id)]) <= JND_THRESHOLD:
                config_dash.LOG.info("VMAF not lower than 1 JND")
                if temp_objFunc > sel_objFunc:
                    ...
                    config_dash.LOG.info("Changing selection: Higher objective function")
