...
def mcom_dash(dp_object, dash_player, current_bitrate, adaptationSetIdx, segment_number):
    ...
    buffer_occupancy = dash_player.segment_duration * (dash_player.buffer.qsize() / dash_player.max_buffer_size)
    ...
    if bitrates:
        ...
        for adaptation in dp_object.adaptationSets:
            ...
            best_condition = float(segment_sizes[int(adaptation.id)]) < selSS and float(vmafs[int(adaptation.id)]) >= selVMAF  # SegmentSize is lower and VMAF is higher
            
            higher_size = False
            if float(segment_sizes[int(adaptation.id)]) > selSS and float(vmafs[int(adaptation.id)]) > selVMAF:
                higher_size = (float(segment_sizes[int(adaptation.id)]) - selSS) / selSS < 0.1 and float(vmafs[int(adaptation.id)]) - selVMAF > JND_THRESHOLD  # SegmentSize is higher by less than 10% but VMAF difference is greater than JND threshold
            
            lower_size = False
            if float(segment_sizes[int(adaptation.id)]) < selSS and float(vmafs[int(adaptation.id)]) < selVMAF:
                lower_size = (selSS - float(segment_sizes[int(adaptation.id)])) / selSS > 0.15 and selVMAF - float(vmafs[int(adaptation.id)]) < JND_THRESHOLD  # SegmentSize is reduced by at least 15% and VMAF difference is lower than JND threshold
            ...
            elif higher_size or lower_size:
                next_adaptation_id = int(adaptation.id)
                next_bitrate = bitrates[next_adaptation_id]
                selSS = float(segment_sizes[next_adaptation_id])
                selVMAF = float(vmafs[next_adaptation_id])
